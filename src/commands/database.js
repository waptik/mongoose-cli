...
exports.handler = async function(args) {
  ...
  switch (command) {
    case 'db:create':
      const options = pick(args, ['charset', 'collate', 'encoding', 'ctype', 'template']);
    ...
    case 'db:drop':
      await sequelize
        .query(`DROP DATABASE ${sequelize.getQueryInterface().quoteIdentifier(config.database)}`, {
          type: sequelize.QueryTypes.RAW,
        })
        .catch(e => helpers.view.error(e));
  ...
  switch (config.dialect) {
    ...
    case 'mysql':
      return (
        ...
        sequelize.getQueryInterface().quoteIdentifier(config.database) +
        (options.charset
          ? ' DEFAULT CHARACTER SET ' + sequelize.getQueryInterface().quoteIdentifier(options.charset)
          : '') +
