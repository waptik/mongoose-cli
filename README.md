...
```sh
Mongoosee CLI [Node: 11.10.0, CLI: 1.0.0, ODM: 5.5.12]
...
By default, mongoosejs-cli generates the migrations,seeders and models directories and files in the root directory of your project. To change this behaviour, create a new file called `.mongooserc` or `mongooserc.json` manually or use the the following command:
...
By default the CLI will try to use the file `config/config.js` and `config/config.json`. You can modify that path either via the `--config` flag or via the option mentioned earlier. Here is how a configuration file might look like (this is the one that `mongoose init` generates):
...
```json
{
  ...
  "production": {
    "database": {
      "url": "mongodb+srv://root:password@subdomain.mongodb.com",
      "options": {
        "useNewUrlParser": true,
        "dbName": "database_production",
      }
...
``` 
$ mongoose model:create --name User --attributes name:String,state:Boolean,birth:Date,card:Number
```
...
```
$ mongoose migration:create --name <migration_name>
```
...
- [Mongoosejs Documentation](http://mongoosejs.com/docs/index.html)
- [CLI Options](#options)
...
- [Sequelize](https://github.com/sequelize) for their [CLI](https://github.com/sequelize/cli) in which Mongoosejs-cli structure was heavily based on.
